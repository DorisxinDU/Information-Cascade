...
def plotfigure(ix_epoch,iy_epoch,epochsInds,title):
    ...
    print('the shape of the hidden layers output is:',I_XT_array.shape)
    I_TY_array=np.array(iy_epoch)
    print('the shape of the hidden layers output is:',I_TY_array.shape)
    cmap = plt.get_cmap('gnuplot')
    colors = [cmap(i) for i in np.linspace(0, 1,len(epochsInds))]
    print('the length of color is'+str(len(colors)))
    for layer_number in range(0,I_XT_array.shape[0]):# #range (0,I_XT_array.shape[0]-1)#[0,1,2,3]
        print('this is layer '+str(layer_number))
        I_XT_array_1=I_XT_array[layer_number]
        I_TY_array_1=I_TY_array[layer_number]
        for index_in_range in range(0,int(len(I_XT_array_1)/interval)):
            # print('current index is :'+str(index_in_range))
            # print('the length of color is'+str(len(colors)))
            XT = I_XT_array_1[index_in_range*interval]
            TY = I_TY_array_1[index_in_range*interval]
            axes.plot(XT, TY, marker='o', linestyle='-', markersize=12,markeredgewidth=0.01,linewidth=0.2,color=colors[index_in_range*interval])
    divider = make_axes_locatable(axes)
    colorbar_axis = divider.append_axes('right', size='5%', pad='3%')
