...
def trainModel(modelToTrain,data,currentEpochs,batch_size,optimizer,accuarcy_name,filepath,structure):
  ...
  hist = trainingResults.history#keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=0, patience=currentEpochs/10, verbose=1, mode='auto'),keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=0, patience=currentEpochs/100, verbose=1, mode='auto'),
  return hist
...
def parameters(number_of_data,num_of_train):
      ...
      structure=[10,7,5,3,2]# The structure  of the network (The number of nodes in each layer)
      outputsize=1          # The dimension of the prediction layer
      num_of_experment='test_relu'+str(structure)+'_regularizers'+str(percent_of_train)+'_full_size_experment_'+str(number_of_data)+'_'+str(num_of_train)  #for change the saving name of results
...
#====================================================================
for number_of_data in ['var_u']:#['arcene','dexter','dorothea','Epileptic','gisette','LSVT','madelon']:#['Epileptic']:#
