...
class OutputObserver(keras.callbacks.Callback):
    ...
    def __init__(self, data_set,layernum,DATA,Label,bins,logs={}):
        ...
        self.Label=Label
        self.bins=bins
        self.local_IXT_cas_layer,self.local_ITY_cas_layer,self.finallocal_IXT_cas_layer,self.finallocal_ITY_cas_layer=[],[],[],[]
        self.KDElocal_IXT_cas_layer,self.KDElocal_ITY_cas_layer,self.KDEfinallocal_IXT_cas_layer,self.KDEfinallocal_ITY_cas_layer=[],[],[],[]
        self.kdislocal_IXT_cas_layer,self.kdislocal_ITY_cas_layer,self.kdisfinallocal_IXT_cas_layer,self.kdisfinallocal_ITY_cas_layer=[],[],[],[]
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        layeroutput_cas=intermediate_layer_model.predict(self.data_set)
        intermediate_predict_layer_model = Model(inputs=model.input,outputs=model.get_layer('Prediction_Layer').output)
        finallayeroutput_cas=intermediate_predict_layer_model.predict(self.data_set)
        self.out_log['eachlayer'].append(layeroutput_cas)
        self.out_log['each_final_layer'].append(finallayeroutput_cas)
#        ==============================================================
        IX_CAS2,IY_CAS2=calculate_lower_mutual_information(layeroutput_cas[0:len(self.DATA)],self.PXs, self.PYs,self.unique_inverse_x,self.unique_inverse_y,self.bins)
        finalIX_CAS2,finalIY_CAS2=calculate_lower_mutual_information(finallayeroutput_cas[0:len(self.DATA)],self.PXs, self.PYs,self.unique_inverse_x,self.unique_inverse_y,self.bins)
#        IX_CAS2=1/np.log(2) *mutual_information((self.DATA,layeroutput_cas[0:len(self.DATA)]), k=4)
#        IY_CAS2=1/np.log(2) *mutual_information((layeroutput_cas[0:len(self.DATA)],np.float64(self.Label)), k=4)
#        finalIX_CAS2=1/np.log(2) *mutual_information((self.DATA,finallayeroutput_cas[0:len(self.DATA)]), k=4)
#        finalIY_CAS2=1/np.log(2) *mutual_information((finallayeroutput_cas[0:len(self.DATA)],np.float64(Label)), k=4)
        # with sess.as_default():
        # IX_CAS2=K.eval(IX_CAS2)
        # IY_CAS2=K.eval(IY_CAS2)
        # finalIX_CAS2=K.eval(finalIX_CAS2)
        # finalIY_CAS2=K.eval(finalIY_CAS2)
...
#        ==============================================================
        IX_CAS,IY_CAS=calculate_mutual_information(layeroutput_cas[0:len(self.DATA)],self.PXs, self.PYs,self.unique_inverse_x,self.unique_inverse_y,self.bins)
        finalIX_CAS,finalIY_CAS=calculate_mutual_information(finallayeroutput_cas[0:len(self.DATA)],self.PXs, self.PYs,self.unique_inverse_x,self.unique_inverse_y,self.bins)
        # with sess.as_default():
        # IX_CAS=K.eval(IX_CAS)
        # IY_CAS=K.eval(IY_CAS)
        # finalIX_CAS=K.eval(finalIX_CAS)
        # finalIY_CAS=K.eval(finalIY_CAS)
...
#        ================================================================================
        IX_CAS1,IY_CAS1=calculate_layer_mutual_information(layeroutput_cas[0:len(self.DATA)],self.PXs, self.PYs,self.unique_inverse_x,self.unique_inverse_y,self.bins)
        finalIX_CAS1,finalIY_CAS1=calculate_layer_mutual_information(finallayeroutput_cas[0:len(self.DATA)],self.PXs, self.PYs,self.unique_inverse_x,self.unique_inverse_y,self.bins)
...
def trainModel(modelToTrain,data,currentEpochs,batch_size,optimizer,accuarcy_name,filepath,layernum,DATA,Label):
    ...
    trainingResults = modelToTrain.fit(data[0],data[1], batch_size=batch_size, nb_epoch=currentEpochs, validation_data=(data[2],data[3]),verbose=1,callbacks=[keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=0, patience=15, verbose=1, mode='auto'),keras.callbacks.ModelCheckpoint(filepath+"/weights-improvement-{epoch:02d}.hdf5", monitor='val_loss', verbose=1, save_best_only=False, save_weights_only=False, mode='auto', period=1),prediction_logs])
...
def parameters(number_of_data,num_of_train):
      ...
      nb_epoch=3000 # The number of training epo
...
for number_of_data in  ['Epileptic']:#['Epileptic']:#,'Epileptic','gisette','LSVT','madelon','arcene','dexter'
