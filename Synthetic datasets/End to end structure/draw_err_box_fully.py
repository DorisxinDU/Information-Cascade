...
def load_DATA(name):
    ...
    with open(name, 'rb') as f:
        data=cPickle.load(f)
        return data
#========================================If you want to draw the results for different datasets and structure, please modify the corresponding parameters===========================================
...
# ======================load accuarcy of fully connected========================================================
        ...
        # =======================The name of the file saing cascade results==============================================
        num_of_experment1=num_of_experment
        num_of_experment2=str(percent_of_train)+str(structure)+'_experment_'+str(number_of_data)+'_'+str(num_of_train)
        # ===================================================================================================================
        for fold in range(fold_num):
            ...
            accuarcy_save_path='history_accuarcy/acc_'+num_of_experment
            history_each_fold['fully_connect_train'].append(np.mean(HISTORY['accuarcy'][-1][accuarcy_name][-average_interval:-1]))
            history_each_fold['fully_connect_test'].append(np.mean(HISTORY['accuarcy'][-1]['val_'+accuarcy_name][-average_interval:-1]))
            cascade_accuarcy_save_path='/home/xd3y15/[10,7,5,3]_generate_dataset/[10,7,5,3]_generate_dataset/history_accuarcy/acc_'+'fold_'+str(fold+1)+num_of_experment2
            cas_HISTORY=load_DATA(cascade_accuarcy_save_path+'/'+'acc.pkl')
            history_each_fold['fully_cascade_train'].append(np.mean(cas_HISTORY['iter'+str(3)][-1][accuarcy_name][-average_interval:-1]))
            history_each_fold['fully_cascade_test'].append(np.mean(cas_HISTORY['iter'+str(3)][-1]['val_'+accuarcy_name][-average_interval:-1]))
            history_each_fold['cascade_layer1_train'].append(np.mean(cas_HISTORY['iter'+str(0)][-1][accuarcy_name][-average_interval:-1]))
            history_each_fold['cascade_layer1_test'].append(np.mean(cas_HISTORY['iter'+str(0)][-1]['val_'+accuarcy_name][-average_interval:-1]))
            history_each_fold['cascade_layer2_train'].append(np.mean(cas_HISTORY['iter'+str(1)][-1][accuarcy_name][-average_interval:-1]))
            history_each_fold['cascade_layer2_test'].append(np.mean(cas_HISTORY['iter'+str(1)][-1]['val_'+accuarcy_name][-average_interval:-1]))
            history_each_fold['cascade_layer3_train'].append(np.mean(cas_HISTORY['iter'+str(2)][-1][accuarcy_name][-average_interval:-1]))
            history_each_fold['cascade_layer3_test'].append(np.mean(cas_HISTORY['iter'+str(2)][-1]['val_'+accuarcy_name][-average_interval:-1]))
# ======================load accuarcy of cascade each layer connected========================================================
    fully_connect_train= history_each_fold['fully_connect_train']
    print(fully_connect_train,'ten fully')
    ...
    print(cascade_third_test,'ten cas')
    data = [fully_connect_train,fully_connect_test,cascade_full_train,cascade_full_test,cascade_first_train,cascade_first_test,cascade_second_train,cascade_second_test,cascade_third_train,cascade_third_test]
    print('this is the data:',np.shape(data))
    fig, ax1 = plt.subplots(figsize=(20,12))
    fig.canvas.set_window_title('A Boxplot Example')
    fig.subplots_adjust(left=0.1, right=0.9, top=0.9, bottom=0.33)
    ...
    plt.suptitle('Comparison of different models performance on train and test sets',fontsize=15)
    ...
    fig.savefig(accuarcy_comparison_image_save_path+'/accuarcy_comparision.png')
    plt.show()