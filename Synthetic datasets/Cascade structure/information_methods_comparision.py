...
#      input_dimension=12
      ...
      num_of_experment='test_KDE_save_cascade_learning'+str(percent_of_train)+'_'+str(structure)+'_experment_'+str(number_of_data)+'_'+str(num_of_train)  #for change the saving name of results
...
# #        ani.save(accuarcy_image_save_path+'/acc_info.mp4')
        ...
        axes.xaxis.set_ticks(np.arange(1.8, 12.8, 2))
        axes.yaxis.set_ticks(np.arange(1.8, 12.8, 2))
        for i in range(len(structure)):
        ...
        axes1.xaxis.set_ticks(np.arange(0.5, 1.02, 0.1))
        axes1.yaxis.set_ticks(np.arange(0.5, 1.02, 0.1))
        ...
        fig.savefig(information_compare_path+'/information_compasion.png',bbox_inches="tight")
        # if not os.path.exists(finalinformation_figure_path):
        #     os.makedirs(finalinformation_figure_path)
